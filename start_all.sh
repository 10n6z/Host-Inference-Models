...
if [[ -d "${FRAMEPACK_ROOT:-/gpt-lab/long/repos/FramePack}" ]]; then
  start_tmux "framepack-server" "source \"/gpt-lab/long/conda/etc/profile.d/conda.sh\" && conda activate host-framepack && cd \"${FRAMEPACK_ROOT:-/gpt-lab/long/repos/FramePack}\" && export OUTPUT_ROOT=\"$OUTPUT_ROOT\" HF_HOME=\"${HF_HOME:-/gpt-lab/long/hf-cache}\" HF_HUB_CACHE=\"${HF_HUB_CACHE:-/gpt-lab/long/hf-cache/hub}\" FRAMEPACK_PUBLIC_BASE_URL=\"$PUBLIC_BASE_URL\" CUDA_VISIBLE_DEVICES=\"${FRAMEPACK_CUDA_VISIBLE_DEVICES:-2}\" && exec uvicorn framepack_server:app --host 0.0.0.0 --port 8012 2>&1 | tee -a \"$LOG_DIR/framepack-server.log\""
else
