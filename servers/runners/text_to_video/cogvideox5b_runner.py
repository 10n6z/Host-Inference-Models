...
class CogVideoX5BRunner:
    def __init__(self):
        self.model_path = os.getenv("COGVIDEOX5B_MODEL_PATH", "/gpt-lab/long/models/text-to-video/cogvideox-5b")
        self.pipe = None
    ...
    def load_model(self):
        ...
        if not os.path.isdir(self.model_path):
            raise FileNotFoundError(f"CogVideoX 5B model folder not found: {self.model_path}")
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is not available. CogVideoX 5B needs GPU inference.")
    ...
    def generate(self, prompt: str, output_path: str, negative_prompt: str | None = None, width: int = 720, height: int = 480, num_frames: int = 49, steps: int = 50, guidance_scale: float = 6.0, seed: int | None = None, fps: int = 8) -> str:
