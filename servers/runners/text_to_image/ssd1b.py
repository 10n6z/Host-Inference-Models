...
class SSD1BRunner:
    def __init__(self):
        self.model_path = os.getenv("SSD1B_MODEL_PATH", "/gpt-lab/long/models/text-to-image/ssd-1b")
        self.pipe = None
    ...
    def load(self):
        ...
        if not os.path.isdir(self.model_path):
            raise FileNotFoundError(f"SSD-1B model folder not found: {self.model_path}")
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is not available. SSD-1B needs GPU inference.")
    ...
    def generate(self, prompt: str, output_path: str, negative_prompt: str | None = None, width: int = 1024, height: int = 1024, steps: int = 30, guidance_scale: float = 9.0, seed: int | None = None, num_images: int = 1) -> str:
        pipe = self.load()
